...
with open('user_ratings.pkl', 'rb') as handle:
    user_dict = pickle.load(handle)
with open('movie_scores.p', 'rb') as handle1:
...
def get_user_score_dict(user_dict,movie_dict):
    ...
    for user in tqdm(user_dict.keys()):
        users_ratings=user_dict[user][0]
        
        user_subspace_score = np.array([0.,0.,0.,0.,0.])
        for movieID,rating in users_ratings.items():
            try:
                movie_subspace_scores = movie_dict[movieID]
...
with open('movie_scores.p', 'wb') as handle1:
